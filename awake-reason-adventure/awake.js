...
class MapObject {
    ...
    constructor(x, y, obj) {
        ...
        this.obstacle = this.obj === 'space' ? false : true;
        // oneOf: space | slash | backslash | X | pipe | minus | plus | pound
        // this.width ??
    }
    whatObj(char) {
        switch (char) {
        case ' ' : return 'space'; // no obstacle with respect to character/hero etc.: others yes, for now
        case '/' : return 'slash';
    ...
    draw(){
        ...
        this.map.split('\n').forEach(m => {
            ...
            for (let i=0; i < m.length; i++) {
                ...
                objRow.push(new MapObject(x_positioning, y-10, char));
            }
...
function init(){
    ...
    mapObjects.draw();
    mapObjects.clearObjXatRowY(0,0);
